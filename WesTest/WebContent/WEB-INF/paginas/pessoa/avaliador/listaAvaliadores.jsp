...
function excluir(id, x){
	$.post("excluir_pessoa", {"id": id}, function(){
		$("#linha_"+id).hide("slow", function(){
			if (x >-1){
				jsonAva.splice(x,1)
				alert("Avaliador "+pessoa.nome+" excluido com sucesso!");
			}
			paginar();
...
var qtdPorPagina = 10;
var pagAtual = 0;
var jsonAva = JSON.parse('${listaAva}');
var tamanho = Object.keys(jsonAva).length;
var avaliador;

function paginar(){
	$('table > tbody > tr').remove();
	var tbody = $('table > tbody');
	tamanho = Object.keys(jsonAva).length;
	for(var x = pagAtual * qtdPorPagina; x < tamanho && x < (pagAtual+1)*qtdPorPagina;x++){
		avaliador = jsonAva[x];
		tbody.append($("<tr id='linha_"+avaliador.idPessoa+"'>")			
			.append($("<td>").append(avaliador.nome))			
			.append($("<td>").append(avaliador.disciplina.descricao))
			.append($("<td>").append("<a href='alterar_avaliador?id="+avaliador.idPessoa+"'><span class='glyphicon glyphicon-pencil'>Alter</span></a>")));	
		}
...
<div class="form col-sm-2 col-sm-offset-8">
	...
	<a href="<c:url value="/admin/form_avaliador"/>" class="btn btn-sm animated-button victoria-two" style="color:#000000;">Novo Avaliador</a>
